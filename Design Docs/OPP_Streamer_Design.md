...
**New in this design:** `SetTrainingStimulusSet(...)` — a single call that queues Subject, Caregiver, and Waver training buffer updates together, guaranteeing they apply at the same boundary. This replaces relying on three separate `SetTrainer` calls arriving close enough together to land in the same drain. **Updated during the build:** this section originally sketched the signature as `Dictionary<string, double[]> byParticipant`; the actual built `StreamerEngine.SetTrainingStimulusSet` instead takes four positional `float[]` buffers (caregiver, waver, subjectBackground, subjectSignal) — simpler, and gives compile-time confidence all four are always provided together. `ConfigApi.SetTrainingStimulusSet` mirrors that shape (as `double[]`) rather than reintroducing the dictionary.
...
4. **ConfigApi** — full surface, ready for MATLAB-side integration and testing. **Done** — `ConfigApi` (new `OppStreamer.ConfigApi` project), built against the design doc's §5.8 method-name list plus the conventions established in Core/Hardware; every place a signature had to be invented rather than carried over from an already-built method is flagged explicitly (see README "ConfigApi (MATLAB-facing surface)") for you to check against the real MATLAB call sites. Verified in-sandbox (27 scenarios, against fake Hardware stand-ins — the real Hardware dependency means it can't build/run in-sandbox itself, same as Stages 1–3's Hardware-dependent pieces); not yet exercised against real MATLAB or real devices.
5. **DiagnosticsView** — real-time stacked plot window. **Done** — `OppStreamer.Diagnostics` (new project): a WinForms window with a running/stopped indicator and one stacked ScottPlot per channel, fed by a new `WaveformMonitor` (Core) that does the §5.7 decimated-snapshot bucketing; owned by `ConfigApi.Initialize()`/`Close()` per §5.7's lifecycle. `WaveformMonitor` itself is fully unit tested (see README); the WinForms/ScottPlot pieces are **unverified** — same sandbox limitation as Hardware/ConfigApi (no Windows, no network for the ScottPlot package) — see README "DiagnosticsView" for what to check on your machine.
