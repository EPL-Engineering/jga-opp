...
///
/// 2026-08-22: updated for the Background/Signal redesign — two-span RenderFrame (Caregiver,
/// Subject), Waver gone.
/// </summary>
public static class StopBoundaryTests
{
    ...
    public static void Register(TestRunner runner)
    {
        ...
        runner.Test("RequestStop's silence lands exactly at the boundary, both participants together", StopSilencesBothAtBoundary);
    ...
    private static void StopSilencesBothAtBoundary()
    {
        const int loopLen = 4;
        var engine = new StreamerEngine();
        engine.Reset(loopLen);
        ...
        engine.RenderFrame(4, c, s); // establish a normal loop first (crosses one boundary, lands back at cursor 0)
        engine.RequestStop();
    ...
    private static void WaitReflectsBoundaryState()
    {
        ...
        engine.RenderFrame(3, c[..3], s[..3]); // crosses the boundary that ends this loop
