...
public sealed class StimulusStore
{
    ...
    ///
    /// CAVEAT if you're setting several participants' buffers together for the currently-selected
    /// mode (e.g. Caregiver, Waver, and both Subject buffers, one call each): each call here queues
    /// independently, and the audio thread can drain and apply whatever's queued so far at ANY
    /// boundary crossing — including one that lands in between two of your calls. That produces one
    /// torn loop pass (part of the group already updated, part still on the old buffers) before the
    /// rest catches up on the following boundary. If the whole group needs to land on the very same
    /// boundary, use <see cref="SetStimulusSet"/> instead — it queues the whole group as one atomic
    /// unit.
    /// </summary>
    ...
    /// currently selected combination — see that method's doc comment for the same torn-update
    /// caveat when setting several buffers as a group one call at a time.
    /// </summary>
    ...
    /// Subject Signal) as a single group. Whichever of these are currently selected for playback
    /// change together, on the same loop boundary — never staggered across separate calls landing
    /// on different wraps, unlike calling <see cref="SetContinuousStimulus"/>/
    /// <see cref="SetSubjectStimulus"/> one at a time for the same group (see their doc comments'
    /// "torn update" hazard — a real audio-thread boundary can land in between two of those calls
    /// and apply only part of the group).
    ///
    ...
    /// an explicit <paramref name="mode"/> once the same one-call-at-a-time tear hazard turned out
    /// to apply identically to Test-mode buffers — see <see cref="SetTrainingStimulusSet"/> below,
    /// kept as a thin Training-specific alias for existing callers.
    /// </summary>
    public void SetStimulusSet(OperatingMode mode, float[] caregiver, float[] waver, float[] subjectBackground, float[] subjectSignal)
    {
        ...
        _continuous[(Participant.Caregiver, mode)] = caregiver;
        _continuous[(Participant.Waver, mode)] = waver;
        _subject[(mode, false)] = subjectBackground;
        _subject[(mode, true)] = subjectSignal;
